...
- 剛體註冊（`rigid_registration`）可調整 Mattes 互資訊與梯度下降參數（直方圖分箱、學習率、迭代與收斂條件）；Demons 多尺度流程加強重採樣尺寸驗證並補充文件說明  
- 提供像素座標與病患座標之間的轉換工具  
- 提供 CT 模態 IOD 輔助，並以 **`CTBuilder`** 從 SimpleITK 影像或 numpy 體資料產生多張切片的 CT DICOM（可沿用參考序列的病患／檢查／序列資訊與常見方位）  
- SimpleITK 影像前處理：**雙邊濾波**與**中值濾波**降噪、**N4 偏壓場校正**（`n4bfc`）  
- 空間／可變形 REG 解析與轉換提取已調整（反矩陣處理、註冊字典去冗餘、位移場網格提取）  
...
- **ct**：CT 影像功能  
  - `builder`：`CTBuilder`，由 `sitk.Image` 或 numpy 體資料建立 CT Image 多切片 DICOM（可帶入參考序列以複製中繼資料）  
  - `ct_ds_iod`：CT IOD 定義  
...
- **utils**：實用工具  
  ...
  - `sitk_transform`：SimpleITK 轉換、`SimpleITKImageBuilder` 與重採樣工具（原點、體素間距、方向可傳 numpy，內部會轉成 SimpleITK 相容格式）  
  - `sitk_image_process`：雙邊／中值降噪與 N4 偏壓場校正（`bilateral_denoise`、`median_denoise`、`n4bfc`）  
